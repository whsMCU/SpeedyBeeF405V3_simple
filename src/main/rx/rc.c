...
uint8_t rcExpo[3] = {0, 0, 0};
uint8_t rcRates[3] = {7, 7, 7};
uint8_t rates[3] = {67, 67, 67};
...
float applyRaceFlightRates(const int axis, float rcCommandf, const float rcCommandfAbs)
{
    ...
    angleRate = angleRate * (1 + rcCommandfAbs * (float)rates[axis] * 0.01f);
...
float applyKissRates(const int axis, float rcCommandf, const float rcCommandfAbs)
{
    ...
    float kissRpyUseRates = 1.0f / (constrainf(1.0f - (rcCommandfAbs * (rates[axis] / 100.0f)), 0.01f, 1.00f));
    float kissRcCommandf = (power3(rcCommandf) * rcCurvef + rcCommandf * (1 - rcCurvef)) * (rcRates[axis] / 1000.0f);
...
#define quickRatesRcExpo 0

float applyQuickRates(const int axis, float rcCommandf, const float rcCommandfAbs)
{
    const uint16_t rcRate = rcRates[axis] * 2;
    const uint16_t maxDPS = MAX(rates[axis] * 10, rcRate);
    const float expof = rcExpo[axis] / 100.0f;
...
#define itermThrottleThreshold 250
uint16_t itermAcceleratorGain = 3500;
#define antiGravityMode ANTI_GRAVITY_SMOOTH
...
static void checkForThrottleErrorResetState(uint16_t rxRefreshRate)
{
    ...
    const int16_t throttleVelocityThreshold = itermThrottleThreshold;
...
//    if (antiGravityMode == ANTI_GRAVITY_STEP) {
//        if (ABS(rcCommandSpeed) > throttleVelocityThreshold) {
//            pidSetItermAccelerator(CONVERT_PARAMETER_TO_FLOAT(itermAcceleratorGain));
//        } else {
...
void processRcCommand(void)
{
    if (isRxDataNew) {
        for (int axis = FD_ROLL; axis <= FD_YAW; axis++) {
            ...
            rawSetpoint[axis] = constrainf(angleRate, -1.0f * 1998, 1.0f * 1998);
        }
...
#define thrMid8 50
#define thrExpo8 0
#define rates_type RATES_TYPE_ACTUAL
...
void initRcProcessing(void)
{
    ...
    for (int i = 0; i < THROTTLE_LOOKUP_LENGTH; i++) {
        const int16_t tmp = 10 * i - thrMid8;//currentControlRateProfile->thrMid8;
        uint8_t y = 1;
        if (tmp > 0)
            y = 100 - thrMid8;
        if (tmp < 0)
            y = thrMid8;
        lookupThrottleRC[i] = 10 * thrMid8 + tmp * (100 - thrExpo8 + (int32_t) thrExpo8 * (tmp * tmp) / (y * y)) / 10;
